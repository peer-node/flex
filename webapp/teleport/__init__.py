...
def teleport_directory():
    if 'TELEPORTDIR' in os.environ.keys():
        return os.environ['TELEPORTDIR']
    return join(expanduser("~"), ".teleport")


def get_config_file():
    return join(teleport_directory(), "teleport.conf")
...
def get_client():
    config = get_config()
    rpc_port = int(config.get("rpcport", 8732))
    user = config.get("rpcuser")
    password = config.get("rpcpassword")
    host = config.get("rpchost", "localhost")
    return HttpClient(url="http://" + host + ":" + str(rpc_port), username=user, password=password)
