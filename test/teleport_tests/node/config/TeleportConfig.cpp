...
uint64_t TeleportConfig::size()
{
    return settings.size();
}

string& TeleportConfig::operator[](string key)
{
...
void TeleportConfig::InterpretNegativeSetting(string key_name)
{
...
void TeleportConfig::ReadFromStream(stringstream &config_stream)
{
    set<string> options;
    options.insert("*");
...
void TeleportConfig::ParseCommandLineArgument(string argument)
{
    if (argument[0] != '-')
        return;
...
void TeleportConfig::ParseCommandLineArguments(int argc, const char **argv)
{
    for (int i = 1; i < argc; i++)
        ParseCommandLineArgument(argv[i]);
}

bool TeleportConfig::Value(string key, bool default_value)
{
...
bool TeleportConfig::Bool(string key, bool default_value)
{
    return Value(key, default_value);
}

uint64_t TeleportConfig::Value(string key, uint64_t default_value)
{
    if (settings.count(key))
    {
...
uint64_t TeleportConfig::Uint64(string key, uint64_t default_value)
{
    return Value(key, default_value);
}

string TeleportConfig::Value(string key, string default_value)
{
    if (settings.count(key))
    {
        return settings[key];
...
string TeleportConfig::String(string key, string default_value)
{
    return Value(key, default_value);
}
