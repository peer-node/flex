...
#include "test/teleport_tests/node/credit/messages/ListExpansionRequestMessage.h"
...
SignedTransaction ValidTransaction(CreditSystem *credit_system, MinedCreditMessage& prev_msg)
{
    ...
    Credit output = Credit(Point(SECP256K1, 5), ONE_CREDIT * 0.5);
                                                                ...
                                                                MinedCreditMessage& prev_msg)
{
    ...
    msg.mined_credit.amount = ONE_CREDIT;
    msg.mined_credit.public_key = Point(SECP256K1, 4);
...
void MakeProofOfWorkInvalid(SimpleWorkProof& proof)
{
    proof.nonce += 1;
}
