...
void RelayMessageHandler::SendRelayJoinMessage(MinedCreditMessage &msg)
{
    auto join_message = Relay().GenerateJoinMessage(data.keydata, msg.GetHash160());
    join_message.Sign(data);
    data.StoreMessage(join_message);
    HandleRelayJoinMessage(join_message);
