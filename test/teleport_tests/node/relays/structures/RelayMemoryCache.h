...
public:
    ...
    std::map<uint160, uint160> latest_mined_credit_message_hashes;
    std::deque<uint160> relay_state_hashes;
    uint32_t maximum_number_of_relay_states{MAX_RELAY_STATES};

    Mutex mutex;

    RelayMemoryCache& operator=(RelayMemoryCache &other);
    ...
    std::vector<uint32_t> ConstructRelayList(RelayState &state);
};
