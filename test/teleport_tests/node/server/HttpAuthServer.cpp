...
struct mhd_coninfo {
    ...
    HttpAuthServer* server;
    int code;
};

HttpAuthServer::HttpAuthServer(int port, const std::string& username, const std::string& password,
...
IClientConnectionHandler *HttpAuthServer::GetHandler(const std::string &url)
{
...
bool HttpAuthServer::StartListening()
{
    if(!this->running)
    {
        if (this->path_sslcert != "" && this->path_sslkey != "")
        {
            try {
                ...
                this->daemon = MHD_start_daemon(MHD_USE_SSL | MHD_USE_SELECT_INTERNALLY, this->port, NULL, NULL, HttpAuthServer::callback, this, MHD_OPTION_HTTPS_MEM_KEY, this->sslkey.c_str(), MHD_OPTION_HTTPS_MEM_CERT, this->sslcert.c_str(), MHD_OPTION_THREAD_POOL_SIZE, this->threads, MHD_OPTION_END);
            }
        ...
        {
            this->daemon = MHD_start_daemon(MHD_USE_SELECT_INTERNALLY, this->port, NULL, NULL, HttpAuthServer::callback, this,   MHD_OPTION_THREAD_POOL_SIZE, this->threads, MHD_OPTION_END);
        }
        if (this->daemon != NULL)
            this->running = true;
...
bool HttpAuthServer::StopListening()
{
    if(this->running)
    {
...
void HttpAuthServer::SetUrlHandler(const string &url, IClientConnectionHandler *handler)
{
...
int HttpAuthServer::callback(void *cls, MHD_Connection *connection, const char *url, const char *method,
                             const char *version, const char *upload_data, size_t *upload_data_size, void **con_cls)
{
    ...
    if (*con_cls == NULL)
    {
        ...
        client_connection->server = static_cast<HttpAuthServer*>(cls);
        *con_cls = client_connection;
        return MHD_YES;
    }
    struct mhd_coninfo* client_connection = static_cast<struct mhd_coninfo*>(*con_cls);

    HttpAuthServer* _this = static_cast<HttpAuthServer*>(cls);
    _this->headers.clear();

    MHD_get_connection_values(connection, MHD_HEADER_KIND, header_iterator, cls);

    if (_this->authentication_required)
    {
        if (_this->headers.count("Authorization") == 0
            or not TimingResistantEqual(_this->headers["Authorization"], "Basic " + _this->username + ":" + _this->password))
        {
            client_connection->code = MHD_HTTP_UNAUTHORIZED;
            client_connection->server->SendResponse("Authorization Failed", client_connection);
...
int HttpAuthServer::header_iterator(void *cls, MHD_ValueKind kind, const char *key, const char *value)
{
    (void)kind;
    HttpAuthServer* _this = static_cast<HttpAuthServer*>(cls);
...
void HttpAuthServer::RequireAuthentication()
{
