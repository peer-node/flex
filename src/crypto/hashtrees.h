...
#define FIELDSIZE (8 * 1024 * 1024)
#define REGIONSIZE (8 * 1024)
...
public:
    vch_t &bits;
    vch_t data;
    uint32_t start, end, number;

    BitRegion(vch_t &bitsIn, uint32_t startIn, uint32_t endIn)
    : bits(bitsIn), start(startIn), end(endIn)
    {
        number = numregions++;
    }
    ...
    void Set(uint64_t position)
    {
        if (position >= start && position < end)
        {
            ...
            throw HashTreeException("region " + to_string(number) + " "
                                    + "inappropriately asked to set position "
                                    + to_string(position));
    }
    ...
    {
        position = position % REGIONSIZE;
        return 1 & (bits[position / 8] >> (position % 8));   
    }

    uint160 Hash()
    {
        uint160 hash = TreeHash(&bits[0] + start / 8, (end - start) / 8);
        return hash;
    }
};
...
static uint32_t numbitfields;

class BitField
{
public:
    vch_t bits;
    std::vector<BitRegion*> regions;
    HashTree<BitRegion> *tree;
    uint32_t length;
    uint32_t number;
    ...
    {
        length = 0;
        bits.resize(FIELDSIZE / 8);
        CreateRegions();
        tree = new HashTree<BitRegion>(regions, REGIONSIZE, false);
        number = numbitfields++;
    }
    ...
    IMPLEMENT_SERIALIZE
    (
        foreach_(const BitRegion* region, regions)
        {
            READWRITE(*region);
            READWRITE(length);
        }
    ...
    void CreateRegions()
    {
        for (uint32_t i = 0; i < FIELDSIZE / REGIONSIZE; i++)
        {
            BitRegion *region = new BitRegion(bits, 
                                              i * REGIONSIZE, 
                                              (i + 1) * REGIONSIZE);
    ...
    uint160 Hash()
    {
        uint160 result = tree->Hash(length);
        return result;
    }
};
