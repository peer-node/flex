...
public:
    ...
    CBigNum(BIGNUM *bn)
    {
        ...
        {
            BN_clear_free(this);
            throw bignum_error("CBigNum::CBigNum(BIGNUM *) : BN_copy failed");
        }
    }
    ...
    void setuint160_(uint160 n160)
    {
        uint256 n256;
        n256.SetHex(n160.GetHex());
        setuint256(n256);
    }

    void setuint160(uint160 n160)
    {
        uint256 n256;
        n256.SetHex(n160.GetHex());
        setuint256(n256);
    ...
    vch_t getvch32()
    {
        uint256 n = getuint256();
        vch_t vch(32);
        memcpy(&vch[0], &n, 32);
        return vch;
    }

    CBigNum& setvch32(vch_t vch)
    {
        uint256 n;
        memcpy(&n, &vch[0], 32);
        setuint256(n);
    ...
    CBigNum InverseMod(CBigNum n)
    { 
        CAutoBN_CTX pctx;
        CBigNum result;
        BN_mod_inverse(&result, this, &n, pctx);
        return result;
    }

    CBigNum& Randomize(CBigNum range)
    {
        BN_rand_range(this, &range);
        return *this;
    }

    CBigNum& Randomize(BIGNUM *range)
    {
        BN_rand_range(this, range);
        return *this;
    }
...
inline CBigNum operator^(const CBigNum& a, const CBigNum& b)
{
    ...
    int32_t shift = 0;
    CBigNum two_b = 2 * b;
    while (BN_cmp(&s, &two_b) <= 0)
    {
        if (BN_is_bit_set(&b, shift))
        {
            if (BN_is_bit_set(&result, shift))
                BN_clear_bit(&result, shift);
            else
                BN_set_bit(&result, shift);
        }
        shift++;
        s = CBigNum(1) << shift;
    }
