#include <src/base/ui_interface.h>
#include "net.h"
#include "net_cnode.h"
...
void Network::SocketHandlerIteration(unsigned int& nPrevNodeCount)
{
    //
    // Disconnect nodes
    //
    ...
    for (auto pnode : vNodesCopy)
    {
        ...
        if (GetTime() - pnode->nTimeConnected > 60)
        {
            if (pnode->nLastRecv == 0 || pnode->nLastSend == 0)
            {
                ...
                std::cout << "net_sockets.cpp: socket no message in first 60 seconds, " << (pnode->nLastRecv != 0) << (pnode->nLastSend != 0) << "\n";
                pnode->fDisconnect = true;
            }
            else if (GetTime() - pnode->nLastSend > 90*60 && GetTime() - pnode->nLastSendEmpty > 90*60)
            {
                LogPrintf("socket not sending\n");
                std::cout << "socket not sending\n";
                pnode->fDisconnect = true;
            }
            else if (GetTime() - pnode->nLastRecv > 90*60)
            {
                LogPrintf("socket inactivity timeout\n");
                std::cout << "socket inactivity timeout\n";
                pnode->fDisconnect = true;
            }
        }
    }
...
void Network::ThreadSocketHandler()
{
    ...
    while (true)
    {
        SocketHandlerIteration(nPrevNodeCount);
        MilliSleep(10);
    }
}

bool Network::StartListening(string& strError)
{
...
bool Network::BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Teleport is probably already running."), addrBind.ToString());
        else
