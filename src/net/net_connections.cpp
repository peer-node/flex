...
void Network::OpenConnectionsIteration(int64_t& nStart)
{
    ProcessOneShot();
...
void Network::ThreadOpenConnections()
{
    ...
    while (true)
    {
        OpenConnectionsIteration(nStart);
        MilliSleep(500);
    }
}

void Network::OpenAddedConnectionsIteration(unsigned int& i)
{
    boost::this_thread::interruption_point();
    ...
    {
        CSemaphoreGrant grant(*semOutbound);
        std::cout << "open connection: " << OpenNetworkConnection(CAddress(vserv[i % vserv.size()]), &grant) << "\n";
        MilliSleep(500);
        boost::this_thread::interruption_point();
    }
}
...
void Network::ThreadOpenAddedConnections()
{
    ...
    {
        OpenAddedConnectionsIteration(i);
