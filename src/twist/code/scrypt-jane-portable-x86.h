#if defined(CPU_X86) && (defined(COMPILER_MSVC) || defined(COMPILER__GCC))
	#define X86ASM

	/* gcc 2.95 royally screws up stack alignments on variables */
	#if ((defined(COMPILER_MSVC) && (COMPILER_MSVC >= COMPILER_MSVC_VS6PP)) || (defined(COMPILER__GCC) && (COMPILER__GCC >= 30000)))
		#define X86ASM_SSE
		#define X86ASM_SSE2
	#endif
	#if ((defined(COMPILER_MSVC) && (COMPILER_MSVC >= COMPILER_MSVC_VS2005)) || (defined(COMPILER__GCC) && (COMPILER__GCC >= 40102)))
		#define X86ASM_SSSE3
	#endif
	#if ((defined(COMPILER_MSVC) && (COMPILER_MSVC >= COMPILER_MSVC_VS2010SP1)) || (defined(COMPILER__GCC) && (COMPILER__GCC >= 40400)))
		#define X86ASM_AVX
		#define X86ASM_XOP
	#endif
	#if ((defined(COMPILER_MSVC) && (COMPILER_MSVC >= COMPILER_MSVC_VS2012)) || (defined(COMPILER__GCC) && (COMPILER__GCC >= 40700)))
		#define X86ASM_AVX2
	#endif
#endif

#if defined(CPU_X86_64) && defined(COMPILER__GCC)
	#define X86_64ASM
	#define X86_64ASM_SSE2
	#if (COMPILER__GCC >= 40102)
		#define X86_64ASM_SSSE3
	#endif
	#if (COMPILER__GCC >= 40400)
		#define X86_64ASM_AVX
		#define X86_64ASM_XOP
	#endif
	#if (COMPILER__GCC >= 40700)
...
#if defined(COMPILER__GCC) && defined(CPU_X86_FORCE_INTRINSICS)
	#define X86_INTRINSIC
	#if defined(__SSE__)
		#define X86_INTRINSIC_SSE
	#endif
...
#if defined(X86_INTRINSIC_SSSE3)
	static const packedelem8 ALIGN22(16) ssse3_rotl16_32bit      = {{2,3,0,1,6,7,4,5,10,11,8,9,14,15,12,13}};
	static const packedelem8 ALIGN22(16) ssse3_rotl8_32bit       = {{3,0,1,2,7,4,5,6,11,8,9,10,15,12,13,14}};
#endif
...
*/
	...
	#define aj(x) __asm {x}
	#define asm_align8 a1(ALIGN22 8)
	#define asm_align16 a1(ALIGN22 16)
...
detect_cpu(void) {
	...
	cpu_vendors_x86 vendor = cpu_nobody; if (vendor) {}
	x86_regs regs;
