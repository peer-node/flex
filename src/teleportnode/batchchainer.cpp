 ...
 */
    ...
    void BatchChainer_::SwitchToBestChain()
    {
        ...
        if (HaveSameCalend(teleportnode.previous_mined_credit_hash, 
                           credit_hash))
            SwitchToChainViaFork(credit_hash);
        else
            SwitchToChainViaCalend(credit_hash);

        teleportnode.calendar = Calendar(credit_hash);
    ...
    {
        while (teleportnode.previous_mined_credit_hash != calend_credit_hash)
            RemoveBatchFromTip();
    }
    ...
    void BatchChainer_::SwitchToChainViaCalend(uint160 credit_hash)
    {
        ...
        log_ << "SwitchToChainViaCalend(): complete" << "\n"
             << "calendar is now " <<  teleportnode.calendar << "\n"
    ...
    void BatchChainer_::SwitchToChainViaFork(uint160 credit_hash)
    {
        ...
        uint160 fork = GetFork(teleportnode.previous_mined_credit_hash,
                               credit_hash, 
                               stepsback);

        while (teleportnode.previous_mined_credit_hash != fork)
        {
            log_ << "SwitchToChainViaFork(): removing "
                 << teleportnode.previous_mined_credit_hash
                 << "\n";
            teleportnode.RemoveBatchFromTip();
        }
    ...
    void BatchChainer_::RemoveBatchFromTip()
    {
        ...
        teleportnode.wallet.UnhandleBatch(prev_batch);
        ...
        foreach_(const uint160& hash, msg.hash_list.full_hashes)
        {
            ...
            {
                SignedTransaction tx = creditdata[hash]["tx"];
                teleportnode.wallet.UnhandleTransaction(tx);
                teleportnode.pit.MarkAsUnencoded(hash);
                HandleTransaction(tx, NULL);
            }
        ...
        teleportnode.previous_mined_credit_hash = preceding_hash;
    }
    ...
    void BatchChainer_::HandleBatch(MinedCreditMessage msg, CNode* peer)
    {
        ...
        if (teleportnode.pit.IsAlreadyEncoded(msg.GetHash160()))
        {
    ...
    void BatchChainer_::HandleNewMostWorkBatch(MinedCreditMessage msg)
    {
        ...
        bool was_digging = teleportnode.digging;
        teleportnode.digging = 0;

        uint160 credit_hash = msg.mined_credit.GetHash160();
        log_ << "switching to new chain\n";
        if (!teleportnode.pit.SwitchToNewChain(msg, NULL))
            return;
        ...
        teleportnode.pit.pool.RemoveMany(msg.hash_list.full_hashes);
        StoreMinedCreditMessage(msg);
        SwitchToChainViaFork(credit_hash);
        teleportnode.wallet.HandleNewBatch(ReconstructBatch_(msg));
        if (msg.mined_credit.batch_number < 500)
        {
            log_ << "wallet is now: " << teleportnode.wallet << "\n";
        }
        HandleNugget(msg.mined_credit);
        log_ << "Balance is now " << teleportnode.wallet.Balance() << "\n";
        teleportnode.pit.pool.already_included.insert(credit_hash);
        teleportnode.digging = was_digging;
    }
                                                ...
                                                CNode* peer)
    {
        ...
        if (!CheckMinedCreditForMissingData(msg, peer))
        {
            ...
            StoreMinedCreditMessage(msg);
            teleportnode.downloader.RequestMissingData(msg, peer);
            return;
        }

        if (!teleportnode.message_validator.ValidateMinedCreditMessage(msg) || !msg.CheckHashList())
        {
    ...
    {
        teleportnode.pit.HandleNugget(mined_credit);
        vector<MinedCreditMessage> non_orphan_nuggets;
    ...
    void BatchChainer_::HandleTransaction(SignedTransaction tx, CNode *peer)
    {
        ...
        {
            if (!(teleportnode.calendar.CreditInBatchHasValidConnection(credit)))
            {
        ...
        if (!TxContainsSpentInputs(tx))
        {
            ...
            teleportnode.pit.HandleTransaction(tx);
