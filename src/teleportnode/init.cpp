...
CLevelDBWrapper teleportdb(GetDataDir_() / "db", 1 << 29);

CDataStore msgdata(&teleportdb, "m");
CDataStore guidata(&teleportdb, "g");
...
CDataStore commanddata(&teleportdb, "M");
CDataStore depositdata(&teleportdb, "p");
...
CDataStore taskdata(&teleportdb, "K");
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: teleport.conf)") + "\n";
    if (hmm == HMM_BITCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: teleportd.pid)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Teleport is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Teleport version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
